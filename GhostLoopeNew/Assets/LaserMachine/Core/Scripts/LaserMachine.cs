...
public class LaserMachine : MonoBehaviour {
    ...
    bool m_assignSparks;
        ...
        // boss poison bomb property
        float m_currLaserCooldown;
        ...
        void OnEnable()
    {
        ...
        for (int i = 0; i < m_currentProperties.m_raysNumber ; i++)
        {
            ...
            m_currLaserCooldown = m_currentProperties.m_laserCooldown;
	...
	void Update () {
        ...
        if (m_currLaserCooldown > 0) m_currLaserCooldown -= Time.deltaTime;

        if (m_currLaserCooldown <= 0)
            {
                // extend laser
                ExtendLaser();
                m_currLaserCooldown = m_currentProperties.m_laserCooldown;
                    ...
                    if (m_active)
                    {
                        ...
                        if (m_currentProperties.m_physicsType == LaserProperties.PhysicsType.Physics3D)
                        {
                            ...
                            if (hitInfo3D.collider)
                            {
                                ...
                                DoAction();
                                */
                                Debug.LogWarning("Laser Hitting Something!");
                                if (hitInfo3D.collider.tag == "Player")
                                {
                                    Debug.LogWarning("Laser Hitting Player!");
                                    hitInfo3D.collider.gameObject.GetComponent<Player>().PlayerReceiveDamage(m_currentProperties.m_laserDamage);
                                }
    ...
    */
    public void StartLaser(float unitLength, float maxLength)
    {
            m_started = true;
            m_currentProperties.m_unitLength = unitLength;
            m_currentProperties.m_targetMaxLength = maxLength;
    }
	
    private void ExtendLaser()
    {
        m_currentProperties.m_maxRadialDistance += m_currentProperties.m_unitLength;
        if (m_currentProperties.m_maxRadialDistance >= m_currentProperties.m_targetMaxLength) m_ended = true; 
    }
    ...
    public bool CheckEnded()
        ...
        {
            m_currentProperties.m_maxRadialDistance = m_currentProperties.m_minRadialDistance;
            m_currLaserCooldown = m_currentProperties.m_laserCooldown;
            m_started = false;
            m_ended = false;
        }
